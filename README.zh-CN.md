...
| `upwsh edit` | 用 nvim 打开 `~/.config/upwsh/user-settings.ps1` |
...
编辑 `~/.config/upwsh/user-settings.ps1`，在内置别名之后加载；也可 `upwsh edit` 用 nvim 打开（没有 nvim 时用 vim）。`upwsh update` 不会覆盖已有文件。卸载时若文件与随包模板不同则留下，未改过的模板随运行时删除。主题一律删除。
