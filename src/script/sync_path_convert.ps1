...
$source = [IO.Path]::Combine($PSScriptRoot, '..', 'lib', 'path_convert.ps1')
$core = [IO.File]::ReadAllText($source).Replace("`r`n", "`n").TrimEnd("`n")
$begin = '# BEGIN GENERATED PATH CONVERTERS (src/lib/path_convert.ps1)'
...
foreach ($name in @('install.ps1', 'uninstall.ps1')) {
    ...
    if ($Check) {
        throw "$name has stale path converters; run src/script/sync_path_convert.ps1"
    }
...
if ($Check) { Write-Output 'Standalone path converters match src/lib/path_convert.ps1.' }
